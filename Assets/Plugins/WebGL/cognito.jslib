...
CognitoSignIn : function(userPoolId, clientId, username, password, gameObject, method ) {
     var poolData = {
...
GetCognitoIdentityCredentials : function(region, userPoolId, idToken, identityPoolId, gameObject, method){
    ...
    var loginMap = {};
    loginMap['cognito-idp.' +  Region + '.amazonaws.com/' +  UserPoolId] =  Pointer_stringify(idToken);
    ...
    AWS.config.credentials.get(function(err) {
        if (err){
            ...
            unityInstance.SendMessage(go, m, '{"AccessKeyId": ' + access + ', "SecretKey":' + secret + ',"SessionToken": ' + session +'}');
        }
    });
}, 
CheckExpiration: function(gameObject, method){
    var go = Pointer_stringify(gameObject);
    var m = Pointer_stringify(method);
    ...
    if (err) {
        console.log(err.message);
        unityInstance.SendMessage(go, m,err.message)
    }
    else {
